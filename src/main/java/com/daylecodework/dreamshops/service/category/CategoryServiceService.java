...
import com.daylecodework.dreamshops.repository.CategoryRepository;
...
public class CategoryServiceService implements ICategoryService {
    ...
    public Category addCategory(Category category) {
        return Optional.of(category).filter(c -> !categoryRepository.existsByName(c.getName()))
                .map(categoryRepository::save).orElseThrow(() -> new AlreadExistException(category.getName()  + " already exists"));
    }
